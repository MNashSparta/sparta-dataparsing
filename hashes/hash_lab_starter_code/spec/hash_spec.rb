...
describe 'Create tests for a hash' do
  ...
  it 'each fruit should have 4 keys' do
    small_fruit_nutrition_details.each do |key,value| expect(value.keys.length).to eq 4 end
  end

  it 'all calories should be between 40 to 150' do
    small_fruit_nutrition_details.each do |key,value| expect(value[:calories]).to be_between(40,150) end
  end
